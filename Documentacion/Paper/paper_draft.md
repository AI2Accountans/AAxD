...
Current enterprise governance and internal control frameworks are grounded in an unsustainable operational assumption: that accounting discrepancies and errors are unavoidable. For decades, companies have relied on siloed databases and retrospective relational ledgers that treat data desynchronization, financial reporting mistakes, and fraudulent activities as acceptable transactional frictions. Under this legacy paradigm, auditors and finance teams function primarily as forensic investigators, relying on ex-post statistical sampling long after capital has been allocated and risks have crystallized. The resulting "reconciliation debt" drains corporate resources, reduces strategic flexibility, and obscures board visibility.

To resolve these inefficiencies, this paper introduces the Accounting and Audit by Design (A&AD) framework. The A&AD model replaces retroactive verification with real-time, deterministic assurance. By extending the "Shift-Left" paradigm to organizational oversight, A&AD enforces regulatory compliance and fiscal constraints directly at the moment of economic genesis—the initial contract or business event. By representing transactions as self-contained, immutable "holons" within Semantic Knowledge Graphs (leveraging W3C standards such as JSON-LD and SHACL), internal control is transformed from a manual human intervention into an intrinsic topological characteristic of the underlying data architecture. This approach systematically eliminates the need for post-factum reconciliation, validates regulatory compliance at the exact point of data ingestion, and enables continuous auditing across the entire population of business events. Consequently, this model shifts the paradigm from simple data processing to a rule-driven architecture, where systemic trust is engineered directly into the data lifecycle rather than verified retrospectively.
...
The Resource-Event-Agent (REA) accounting framework, introduced by McCarthy (1982), argued that accounting records must reflect the economic substance of transactions rather than just their accounting artifacts. The REA model focuses on the fundamental nature of economic events: the participants (Agents), the values traded (Resources), and the transactions themselves (Events). Subsequent researchers (e.g., Fischer-Pauzenberger & Schwaiger, 2017) have formalized REA into a rigorous ontology (OntoREA) using foundational frameworks like the Unified Foundational Ontology (UFO). However, deploying this ontological design in a practical, scaleable environment requires transitioning away from traditional relational database engines toward Semantic Knowledge Graphs. Graph databases are uniquely suited to preserving complex, multi-dimensional connections without the constraints of rigid schemas.

### 2.2. Model-Driven Systems and XBRL GL Interoperability
...
### 3.3. The Living Knowledge Wrapper (DataBooks)
