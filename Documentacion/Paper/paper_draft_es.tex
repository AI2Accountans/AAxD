...
Para resolver estas ineficiencias, este artículo introduce el marco de Contabilidad y Auditoría por Diseño (A\&AD). El modelo A\&AD reemplaza la verificación retrospectiva por un aseguramiento determinista en tiempo real, logrando un ``Cumplimiento Defendible'' (\textit{Defensible Compliance}) en el momento exacto en que ocurren los eventos. Al extender el paradigma de desplazamiento hacia la izquierda (\textit{shift-left}) al control organizacional, A\&AD hace cumplir las restricciones fiscales y normativas directamente en el momento del origen económico: el contrato inicial o evento de negocio. Al representar las transacciones como holones autónomos e inmutables dentro de grafos de conocimiento semántico, aprovechando estándares W3C como JSON-LD y SHACL \cite{w3crdfval2013}, el control interno deja de ser una intervención humana manual para convertirse en una propiedad topológica intrínseca de la arquitectura de datos. Este enfoque elimina sistemáticamente las conciliaciones posteriores, valida el cumplimiento regulatorio en el punto exacto de ingesta de datos y permite una auditoría continua sobre la totalidad de los eventos comerciales.
\end{abstract}

\vspace{1em}
\noindent \textbf{Declaraci\'on de IA Generativa y Tecnolog\'ias Asistidas por IA en el Proceso de Escritura}\\
Durante la preparaci\'on de este trabajo, el autor utiliz\'o modelos de IA generativa exclusivamente con el prop\'osito de estructurar y traducir ideas originales al ingl\'es. Toda la conceptualizaci\'on, desarrollo intelectual y marco te\'orico son creaci\'on exclusiva del autor. Tras utilizar esta herramienta, el autor revis\'o y edit\'o el contenido seg\'un fue necesario, y asume total responsabilidad sobre el contenido de la publicaci\'on.
...
Además, introducimos el uso de DataBooks híbridos \cite{cagle2026} como mecanismo de transporte estandarizado. Basándonos en el concepto de aprendizaje zero-shot mediante ``códigos de salida semánticos'' \cite{palatucci2009}, incrustamos las taxonomías SKOS \cite{w3cskos} directamente en los modelos de lenguaje grande (LLM) para que actúen como estos límites semánticos estrictos. Esto permite la auditoría inmediata y \textit{zero-shot} de eventos de negocio, tanto por parte de supervisores humanos como de agentes de IA autónomos. La arquitectura resultante es la de una caja de cristal: toda la actividad económica es rastreable, verificable y auditable en tiempo real. El resto de este artículo describe los fundamentos teóricos de este modelo, detalla sus componentes de diseño y demuestra su aplicación práctica en la auditoría del Momento~0 de una entidad corporativa.
...
\bibitem{w3crdfval2013}
W3C. (2013). \textit{RDF Validation Workshop: Practical Assurances for Quality RDF Data}. Cambridge, MA, 10-11 de Septiembre de 2013. Obtenido de \url{https://www.w3.org/2012/12/rdf-val/Overview}
