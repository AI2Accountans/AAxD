...
public class DataElement extends StructureItem {
	...
	public boolean read (Parser.Context context) {
		...
		if (context.getParser().getEDIKind() == EDISettings.EDIStandard.EDIFixed) {
			...
			sError.append(context.getValidator().validate(context.getParent().getParticle().getName(), context.getParticle().getName(), sBuffer.toString()));
		...
		// edi validate
		sError.append(context.getValidator().validate( context.getParent().getParticle().getName(), context.getParticle().getName(), s.toString()));
	...
	public void write (Writer writer, ITextNode node, Particle particle) throws IOException {
		...
		//edi validate
		if (node != null)
			sError.append(writer.getValidator().validate( node.getParent().getName(), nodeName, sValue));
		if (sError.length() > 0)
