...
public class EDIFactDataCompletion extends DataCompletion {
		...
		protected void completeGroup(ITextNode group, Particle groupParticle) {
			...
			for (String sMessageType : m_Document.getMessageTypes()) {
				...
				for (int i=0; i< messages.size(); ++i) {
					...
					completeMessage(m_Document.getMessage(sMessageType), messages.getAt(i), messageParticle);
				}
			}
			ITextNodeList messages  = group.getChildren().filterByName("Message");
			for (int i=0; i< messages.size(); ++i)
				completeMessage(m_Document.getMessage(mSettings.getMessageType()), messages.getAt(i), m_Document.getFirstMessage().getRootParticle());
		...
		protected void completeMessage(Message message, ITextNode messageNode, Particle messageParticle) {
            String sMessageType = message.getMessageType();
            int pos = sMessageType.indexOf('_');
            if (pos >= 0)
                sMessageType = sMessageType.substring(0, pos);
			...
			ITextNode header = makeSureExists(messageNode, messageHeader.getName());
			ITextNode trailer = makeSureExists(messageNode, messageTrailer.getName());
