...
public abstract class DataCompletion {
	...
	protected void completeConfigRules(ITextNode node, Particle particle) {
		completeRules(node, particle, new Vector<Particle>());
	}

	protected ITextNode completeRules(ITextNode node, Particle particle, Vector<Particle> path) {
		path.add(particle);

		if (particle.getNode().getNodeClass() == ITextNode.DataElement) {
			if (node == null || node.getValue().trim().length() == 0) {
				if (particle.getNode().getInlineRule() != null && particle.getNode().getInlineRule().getComplete()) {
					if (node == null)
						node = new TextNode(null, particle.getName(), particle.getNode().getNodeClass());
					else
						node.setValue(particle.getNode().getInlineRule().getExpression().evaluate(node));
				}
				else if (particle.getNode().getRules() != null) {
					for (DataRule rule : particle.getNode().getRules()) {
						if (rule.getComplete() && rule.getReference().matchesParticles(path)) {
							if (node == null)
								node = new TextNode(null, particle.getName(), particle.getNode().getNodeClass());
							else
								node.setValue(rule.getExpression().evaluate(node));
						}
					}
				}
			}
			...
			for (int i = 0; i < particle.getNode().getChildren().length; ++i) {
				Particle childParticle = particle.getNode().getChildren()[i];
				TextNodeList nodes = null;
				if (node != null) nodes = node.getChildren().filterByName(childParticle.getName());
				if (nodes != null && nodes.size() > 0) {
					for (int j = 0; j < nodes.size(); ++j) {
						ITextNode childNode = nodes.getAt(j);
						completeRules(childNode, childParticle, path);
					}
				}
				else if (childParticle.getMinOccurs() > 0 && childParticle.getNode().getNodeClass() != ITextNode.Select && childParticle.getName() != "Message") {
					ITextNode childNode = completeRules(null, childParticle, path);
					if (childNode != null) {
						if (node == null)
							node = new TextNode(null, particle.getName(), particle.getNode().getNodeClass());
	...
	protected boolean completeMandatory(ITextNode dataNode, Particle particle) {
		...
		if (currentItem.getNodeClass() == ITextNode.Settings)
			return true; // do not remove empty separator
			...
			for (int i= 0; i< currentItem.getChildCount(); i++) {
				...
				if (currentItem.getNodeClass() == ITextNode.Group && childParticle.getNode().getNodeClass() != ITextNode.Select && !name.equals("Message")) {
					for (; occurs < childParticle.getMinOccurs(); ++occurs) {
						...
						if (completeMandatory(node, childParticle))
							anyExisted = true;
					}
				}
			}
